  ...
  return (
    <Flex flexWrap="wrap" justifyContent="center" alignItems="center" m="10">
      ...
      <Box p="5">
        ...
        <Text fontSize="lg" pt="3" pb="3" color="grey.700">{desc1}<br />{desc2}</Text>
...
export default function Home({ propertiesForRent, propertiesForSale }) {
  return (
    <Box>
      ...
      />
      <Flex flexWrap='wrap'>
        {propertiesForRent.map(p => <Property property={p} key={p.id} />)}
      </Flex>
      <Banner
        purpose='BUY A HOME'
      ...
      />
      <Flex flexWrap='wrap'>
        {propertiesForSale.map(p => <Property property={p} key={p.id} />)}
      </Flex>
    </Box>
  )
}
